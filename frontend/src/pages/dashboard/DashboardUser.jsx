...
const DashboardUser = () => {
  ...
  const chargerDonnees = async () => {
    ...
    try {
      ...
      const [inscriptionsRes, evenementsRes] = await Promise.all([
        api.get('/participations/mes-inscriptions'),
        api.get('/evenements'),
      ]);

      setMesInscriptions(inscriptionsRes.data.participations || []);
      setEvenementsDispos(evenementsRes.data.evenements || []);

      // Coupons de l'utilisateur
      try {
        const recompensesRes = await api.get('/recompenses/mes-coupons');
        setMesRecompenses(recompensesRes.data.coupons || []);
      } catch {
        setMesRecompenses([]);
      }
