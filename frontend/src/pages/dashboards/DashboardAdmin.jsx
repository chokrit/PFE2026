      ...
      <main className="dash-main">
        ...
        <nav className="admin-nav">
          ...
          {[
            ...
            { key:'equipes',     label:'Équipes IA',      icon:'👥', count:0, color:'#00d4ff' },
        ...
        {activeTab==='equipes' && (
          <div>
            ...
            <div style={{ background:'#12122a', border:'1px solid #2a2a4a', borderRadius:14, padding:'1.25rem', marginBottom:24 }}>
              <h3 style={{ fontSize:13, fontWeight:700, color:'#e8e8f0', marginBottom:14 }}>Paramètres de génération</h3>
              <div style={{ display:'flex', gap:12, flexWrap:'wrap', alignItems:'flex-end' }}>
                <div style={{ flex:2, minWidth:200 }}>
                  <label style={{ display:'block', fontSize:12, color:'#8888aa', marginBottom:6 }}>Événement *</label>
                  <select
                    value={equipeEventId}
                  ...
                  >
                    ...
                    {evenements.filter(ev => ev.stat_event === 'publié').map(ev => (
                      <option key={ev._id} value={ev._id}>{ev.title_event}</option>
                    ))}
                  </select>
                </div>
                <div style={{ flex:1, minWidth:120 }}>
                  <label style={{ display:'block', fontSize:12, color:'#8888aa', marginBottom:6 }}>Membres / équipe</label>
                  <input
            ...
            {equipesSuggeres.length > 0 && (
              <div>
                  ...
                  {equipesSuggeres.map((eq, eqIdx) => (
                    <div key={eqIdx} style={{ background:'#12122a', border:'1px solid #2a2a4a', borderRadius:14, padding:'1rem' }}>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                        ...
                        {!eq.complet && (
                          <div style={{ fontSize:11, color:'#ff6b00', textAlign:'center', padding:'4px 0' }}>⚠ Équipe incomplète</div>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{ display:'flex', gap:12, justifyContent:'flex-end', paddingTop:12, borderTop:'1px solid #2a2a4a' }}>
                  <button onClick={() => setEquipesSuggeres([])} className="dash-btn-ghost" disabled={savingEquipes}>
                    Annuler
                  </button>
                  <button
            ...
            {!loadingEquipes && equipesSuggeres.length === 0 && (
              <div className="dash-empty">
                ...
                <p className="dash-empty__text">
                  Sélectionnez n'importe quel événement publié et cliquez sur "Générer" pour obtenir des suggestions d'équipes.
                </p>
              </div>
            )}
          </div>
